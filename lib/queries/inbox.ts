...
async function fetchCentralTeamInbox(user: SessionUser): Promise<InboxData> {
  const prScope = warehouseScopeForUser(user);
  const poScope = nestedPurchaseRequestWarehouseScope(user);
  const grnScope = nestedPurchaseOrderWarehouseScope(user);

  const [vendorRequests, awaitingPoPrs, pendingCatalog, exceptions, atRiskPos, recentActivity] =
    await dbSerial(
      () => prisma.vendorRequest.findMany({
        where: { status: "PENDING" },
        select: {
  ...
  const configurePoItems: InboxItem[] = awaitingPoPrs.map((pr) => ({
    id: pr.id,
    key: `po_cfg:${pr.id}`,
    kind: "po_to_receive",
    ref: pr.id,
    title: pr.vendor?.businessName ?? prTitle(pr.lines[0]),
    subtitle: "Approved — configure purchase order",
    timestamp: pr.updatedAt.toISOString(),
    href: `/purchase-orders/configure/${pr.id}`,
    actionLabel: "Configure PO",
  }));

  const catalogItems: InboxItem[] = pendingCatalog.map((item) => ({
    id: item.id,
    key: `catalog_pend:${item.id}`,
  ...
  const recentItems: InboxItem[] = recentActivity.map((pr) => ({
    id: pr.id,
    key: `recent_pr:${pr.id}`,
    kind: "pr_pending_approval",
    ref: pr.id,
    ...
    groups: [
      {
        id: "awaiting",
        label: "Awaiting your action",
        description: "PO configuration, vendors, catalog items, and items to review.",
        items: awaiting.slice(0, ITEMS_PER_GROUP),
        total: awaiting.length,
      },
      {
        id: "at_risk",
        label: "At risk",
        description: "POs blocked by an invoice mismatch (Ops Head resolves).",
        items: atRiskItems.slice(0, ITEMS_PER_GROUP),
        total: atRiskItems.length,
      },
      {
        id: "recent",
        label: "Recently updated",
        description: "Recent procurement activity across your warehouses.",
        items: recentItems.slice(0, ITEMS_PER_GROUP),
        total: recentItems.length,
      },
    ],
...
async function fetchInboxForRole(user: SessionUser): Promise<InboxData> {
  switch (user.role) {
    ...
    case Role.CENTRAL_TEAM:
      return fetchCentralTeamInbox(user);
    case Role.OPS_HEAD:
